...
contract SpaceCoinICO is ReentrancyGuard {
     ...
     */
    function toggleFundraisingEnabled() external onlyOwner {
     ...
     */
    function addAddressesToWhitelist(address[] memory _whitelist) external onlyOwner {
        _whitelistAddresses(_whitelist);
    }
     ...
     */
    function moveToPhase(uint8 phase) external onlyOwner nonReentrant {
    ...
    function invest() external payable nonReentrant {
        ...
        uint256 totalInvested = msg.value + preOpenInvestorsBalances[msg.sender]; // gas optimization
        if (phases[currentPhase].shouldReleaseTokens) {
            _transferTokens(msg.sender, totalInvested);
        } else {
            preOpenInvestorsBalances[msg.sender] = totalInvested;
        }
    }
